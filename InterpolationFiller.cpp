...
namespace EFTDEM {
	void InterpolationFiller::fill(const PointCloud &pointCloud, SYCLState &syclState, const unsigned kernelRadius, const int debug) {
		std::cout << "Filling point cloud " << kernelRadius << " times using interpolated filling...\n";
		...
		syclState.queue.submit([&](sycl::handler &handler) {
			...
			handler.parallel_for(sycl::range<2>{pointCloud.height, pointCloud.width}, [=](const sycl::item<2> &item) {
				...
				for (auto x = -static_cast<int>(kernelRadius); x <= static_cast<int>(kernelRadius); ++x) {
					for (auto y = -static_cast<int>(kernelRadius); y <= static_cast<int>(kernelRadius); ++y) {
